...
def _compile_regex(string):
    ...
    if string == "T2*":
        string = "T2\*"
        regex = re.compile(
            r"(\b%s\b)|(_%s_)|(_%s)|(%s_)|(t2star)" % (string, string, string, string),
            re.IGNORECASE,
        )
    # Prevent T2 from capturing T2*
    elif string == "T2":
        regex = re.compile(
            r"(\b%s\b)|(_%s_)|(_%s$)|(%s_)" % (string, string, string, string),
            re.IGNORECASE,
        )
    else:
        regex = re.compile(
...
def is_anatomy_t2(label):
    regexes = [re.compile("t2[^*]*$", re.IGNORECASE)]
    return regex_search_label(regexes, label)


# Aanatomy, Inplane
def is_anatomy_inplane(label):
    regexes = [re.compile("inplane", re.IGNORECASE)]
    return regex_search_label(regexes, label)
...
def is_coil_survey(label):
    regexes = [re.compile("(?=.*coil)(?=.*survey)", re.IGNORECASE)]
    return regex_search_label(regexes, label)
